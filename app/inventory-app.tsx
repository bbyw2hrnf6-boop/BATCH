...
import { ArrowLeft, ArrowRight, BookOpen, Check, CheckCheck, ChevronRight, CircleHelp, ClipboardList, FlaskConical, Globe2, Download, Info, Layers3, Loader2, Minus, Pencil, Plus, RotateCcw, Wine, X } from 'lucide-react';
...
export default function InventoryApp() {
  ...
  const copyList = async () => {
    const text = [t("BATCH · Zutaten in erfassten Pre-Batches"), t('{count} von {total} Pre-Batches erfasst', { count: counted, total: state.recipes.length }), results.some(r => r.demo) ? t("ACHTUNG: Enthält Demo-Rezepte.") : '', t('Zutat | Noch im Batch cl | Entspricht Originalprodukt cl | Originalflasche ml | Rechnerisch volle Flaschen | Rechnerischer Rest cl'), ...shownResults.map(r => `${ingredientName(r)} | ${r.estimated ? t('ca. ') : ''}${format(r.totalCl)} | ${r.stockCl === null ? t('Umrechnung offen') : format(r.stockCl)} | ${r.bottleMl === null ? t('Offen') : format(r.bottleMl)} | ${r.bottles ?? '—'} | ${r.remainderCl === null ? '—' : format(r.remainderCl)}`), '', t("Nur Pre-Batches. Separat vorhandene Originalflaschen sind nicht enthalten. Werte gerundet auf 0,01 cl.")].filter(Boolean).join('\n');
