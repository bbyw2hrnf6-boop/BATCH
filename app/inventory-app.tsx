  ...
  return <main className="app-shell">
    ...
    <Tabs value={tab} onValueChange={value => { setTab(value); setSelected(null); window.scrollTo(0, 0); }} className="app-tabs">
      ...
      <TabsContent value="results">
        ...
        {shownResults.length > 0 && <div className="mobile-results">{shownResults.map(row => <article className="mobile-result-card" key={row.id}>
          <button type="button" className="mobile-result-title product-link" disabled={disabled} onClick={() => setIngredientEdit(row)}>{ingredientName(row)}<Pencil size={16} /></button>
          <p>{row.bottleMl === null ? t('Flaschengröße offen') : t('Originalflasche') + ' ' + format(row.bottleMl) + ' ml'} · {row.estimated ? t('ca. ') : ''}{format(row.totalCl)} cl {t('im Batch')}</p>
          <dl>
            <div><dt>{t('Flaschen')}</dt><dd>{row.bottles ?? '—'}</dd></div>
            <div><dt>{t('Rest cl')}</dt><dd>{row.remainderCl === null ? '—' : format(row.remainderCl)}</dd></div>
            <div><dt>{t('Originalprodukt cl')}</dt><dd>{row.stockCl === null ? '—' : <>{row.estimated && <small>{t('ca.')} </small>}{format(row.stockCl)}</>}</dd></div>
          </dl>
          <span className="result-sources">{row.sources.map(s => s.name).join(' · ')}</span>
        </article>)}</div>}
